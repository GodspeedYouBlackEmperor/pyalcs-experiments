...
class AnalyzerConfiguration:
    def __init__(self, BASE_NAME, EXP_NAME, ENV_NAME, FRIENDLY_ENV_NAME,
                 ...
                 PRIMARY_MARKERS=['*', 'x', '4', '^', 'd'],
                 SECONDARY_MARKERS=['*', 'x', '4', '^', 'd'],
                 PRIMARY_MARKER='o',
                 SECONDARY_MARKER='o',
                 MARKERS_NUMBER=70,
                 FIG_SIZE=[10, 10],  # CHANGE
                 LINE_WIDTH=2,
                 TITLE_TEXT_SIZE=24,
                 AXIS_TEXT_SIZE=18,
                 LEGEND_TEXT_SIZE=16,
...
class Analyzer:
    def __init__(self, acs2_data, acs2er_data, config: AnalyzerConfiguration, acs2_data_full=None, acs2er_data_full=None) -> None:
        ...
        self.metrics = {
            ...
            SPECIFICITY_METRIC: ['Specificity', 'TRIAL', 'SPECIFICITY', 'specificity'],
            CLASSIFIERS_METRIC: ['Classifiers numerosity (num) and reliable (rel)', 'TRIAL', 'NUMBER OF CLASSIFIERS', 'classifiers'],
            STEPS_METRIC: ['Steps', 'TRIAL', 'STEPS', 'steps'],
            REWARD_METRIC: ['Reward', 'TRIAL', 'REWARD', 'reward'],
            AVG_STEPS_METRIC: ['Avg Steps', 'TRIAL',
                               'STEPS', 'avg_steps'],
            OPTIMAL_METRIC: ['OPTIMAL ACTIONS', 'TRIAL', 'OPTIMAL [%]', 'optimal'],
            OPTIMAL_RELIABLE_METRIC: ['OPTIMAL RELIABLE ACTIONS', 'TRIAL', 'OPTIMAL [%]', 'optimal_reliable'],
        }
    ...
    def get_classifiers_acs2(self):
        return self.acs2_data[1]
    ...
    def __plot_attribute(self, explore_metrics_func, exploit_metrics_func, metrics, attribute_name: str, label: str, explore_color: str, exploit_color: str, width: float, marker: str):
        ...
        # plt.plot(x_axis_exploit, exploit, c=exploit_color,  TODO
        # linewidth=width, marker=marker, markevery=density)
    ...
    #     t = self.config.FRIENDLY_ENV_NAME
    #     plt.title(self.config.FRIENDLY_ENV_NAME,
    #               fontsize=self.config.TITLE_TEXT_SIZE)  # CHANGE

    #     if(name == "steps_explore_with_dqn"):
    #         plt.ylim([3, 47])
    ...
    #     if t == "MAZE T3":
    #         plt.legend(fontsize=self.config.TITLE_TEXT_SIZE)

    #     if t == "MAZE 4" or t == "MAZE 5" or t == "MAZE 7" or t == "MAZE X1" or t == "MAZE X2":
    #         plt.xlabel(x_label, fontsize=self.config.AXIS_TEXT_SIZE + 6)

    #     if t == "MAZE 4" or t == "MAZE F1":
    #         plt.ylabel(y_label, fontsize=self.config.AXIS_TEXT_SIZE + 6)
    #     plt.xticks(fontsize=self.config.TICKS_TEXT_SIZE + 5)
    #     plt.yticks(fontsize=self.config.TICKS_TEXT_SIZE + 5)
    ...
    def __get_metric_record_single(self, index, data):
        ...
        return (self.__get_label(m), self.config.PRIMARY_COLORS[index], self.config.PRIMARY_COLORS[index], self.config.PRIMARY_MARKERS[index], self.config.PRIMARY_MARKERS[index]), metric

    def __get_metric_record_double(self, index, data):
        m, (metric, _, _) = data

        return (self.__get_label(m), self.config.PRIMARY_COLORS[index], self.config.SECONDARY_COLORS[index], self.config.PRIMARY_MARKERS[index], self.config.SECONDARY_MARKERS[index]), metric
...
################################### STATISCTIS ###############################
    ...
    def compare_reward_exploit_average_welch_test(self):
        ...
        for i in range(len(info)):
            ...
            test_results.append(row_test_results)
        self.cache_data(info, 'reward_exploit')
        self.__print_results([i[0] for i in info],
    ...
    def compare_knowledge_above_threshold_welch_test(self, threshold):
        ...
        self.cache_data(info, 'knowledge95')
        self.__print_results([i[0] for i in info],
    ...
    def __get_label(self, m):
        if m is int and m < 0 or type(m) is int:
            return ACS2_NAME
...
#############
    ...
    def cache_data(self, info, file_name):
        base_path = os.path.join(
            self.config.RESULTS_PATH, 'bayes_data', file_name)

        if not os.path.isdir(base_path):
            os.makedirs(base_path)

        for i in range(len(info)):
            data = np.array(info[i][1])
            path = os.path.join(base_path, f'{info[i][0]}.npy')
            np.save(path, data)

    # def run_bayes(self, info, file_name, draws=100_000):

    #     for i in range(len(info)):
    #         model = self.bayes_estimate(np.array(info[i][1]), draws)
    #         path = os.path.join(
    #             self.config.RESULTS_PATH, f'{self.__get_label(info[i][0])}_{file_name}_bayes.dill')
    #         with open(path, 'wb') as f:
    #             dill.dump(model, f)

    # def bayes_estimate(self, data: np.ndarray, draws=100_000):
    ...
    def __get_metric_record_single_steps_for_dqn(self, index, data, start_index, end_index):
        m, (metric, _, _) = data

        return (self.__get_label(m), self.config.PRIMARY_COLORS[index], self.config.PRIMARY_COLORS[index], self.config.PRIMARY_MARKERS[index], self.config.PRIMARY_MARKERS[index]), np.array(self.__get_explore_metrics(metric)[STEPS_METRIC].iloc[start_index:end_index])

    def plot_steps_explore_with_dqn(self, moving_avg, start_index, end_index):
        base_path = os.path.join('RESULTS', 'RL_RESULTS')
        dqn_path = os.path.join(
            base_path, 'DQN', self.config.ENV_NAME)
        sq_path = os.path.join(base_path, 'SQ', self.config.ENV_NAME)

        dqn_steps_all = []
        for f in os.listdir(dqn_path):
            full_file_path = os.path.join(dqn_path, f)
            dqn_steps_all.append(np.load(full_file_path)
                                 [start_index:end_index])

        sq_steps_all = []
        for f in os.listdir(sq_path):
            full_file_path = os.path.join(sq_path, f)
            sq_steps_all.append(np.load(full_file_path)[start_index:end_index])

        dqn_steps = np.mean(np.array(dqn_steps_all), axis=0)
        sq_steps = np.mean(np.array(sq_steps_all), axis=0)

        dqn_steps_avg = np.mean(np.array(dqn_steps_all), axis=1)
        sq_steps_avg = np.mean(np.array(sq_steps_all), axis=1)

        self.cache_data(
            [('DQN', dqn_steps_avg), ("SQ", sq_steps_avg)], 'steps')

        acs2_metric, _, _ = self.acs2_data

        metrics_list = list(map(lambda d: self.__get_metric_record_single_steps_for_dqn(
            d[0], d[1], start_index, end_index), enumerate(self.acs2er_data)))
        metrics_list.insert(0, ((ACS2_NAME, self.config.PRIMARY_COLOR,
                                 self.config.PRIMARY_COLOR,
                                 self.config.PRIMARY_MARKER,
                                 self.config.PRIMARY_MARKER,), np.array(self.__get_explore_metrics(acs2_metric)[STEPS_METRIC].iloc[start_index:end_index])))
        metrics_list.append((('Q-Learning', self.config.PRIMARY_COLORS[3],
                              self.config.PRIMARY_COLORS[3],
                              self.config.PRIMARY_MARKERS[3],
                              self.config.PRIMARY_MARKERS[3],), sq_steps))
        metrics_list.append((('DQN', self.config.PRIMARY_COLORS[4],
                              self.config.PRIMARY_COLORS[4],
                              self.config.PRIMARY_MARKERS[4],
                              self.config.PRIMARY_MARKERS[4],), dqn_steps))
        ...
        def __plot_attribute_with_dqn(metric_data, width):
            ((label, c, _, m, _), metric) = metric_data
            metric = moving_average(metric, moving_avg)
            x_axis = range(1, len(metric) + 1)

            density = max(1, int(len(metric) / self.config.MARKERS_NUMBER))
            plt.plot(x_axis, metric, c=c,
                     linewidth=width, marker=m, markevery=density, label=label)

        self.__plot([__plot_attribute_with_dqn], metrics_list,
                    'STEPS EXPLORE', 'TRIAL', 'STEPS', 'steps_explore_with_dqn', 0.5)
        # acs2_metrics, _, _ = self.acs2_data
        # acs2er_metrics, _, _ = self.acs2er_data
        # er_metrics = acs2er_metrics[0][1]
        # eer_metrics = acs2er_metrics[1][1]
