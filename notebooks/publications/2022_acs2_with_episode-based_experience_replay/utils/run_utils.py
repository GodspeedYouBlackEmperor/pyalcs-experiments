...
class Runner:
    ...
    def run_experiment_explore(self, agent: Agent, env, explore_trials, data_path=''):
        # Explore the environment
        explore_metrics = agent.explore(
            env, explore_trials, decay=True, decay_steps=explore_trials * 5)

        self.__save_experiment_data(
            agent, env, explore_metrics, explore_metrics, data_path)
